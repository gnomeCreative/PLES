...
subroutine update
    ...
    do kk=1,1-kp
        ...
        if (myid.eq.nproc-1) then
            call MPI_SENDRECV(u(1,1,jz),1, &
                plantypee,0,11, &
                u(1,1,jz+1),1, &
                plantypee,0,12, &
                MPI_COMM_WORLD,status,ierr)

            call MPI_SENDRECV(v(1,1,jz),1, &
                plantypee,0,13, &
                v(1,1,jz+1),1, &
                plantypee,0,14, &
                MPI_COMM_WORLD,status,ierr)

            call MPI_SENDRECV(w(1,1,jz),1, &
                plantypee,0,15, &
                w(1,1,jz+1),1, &
                plantypee,0,16, &
                MPI_COMM_WORLD,status,ierr)
            ...
            call MPI_SENDRECV(u(1,1,1),1, &
                plantypee,nproc-1,12, &
                u(1,1,0),1, &
                plantypee,nproc-1,11, &
                MPI_COMM_WORLD,status,ierr)

            call MPI_SENDRECV(v(1,1,1),1, &
                plantypee,nproc-1,14, &
                v(1,1,0),1, &
                plantypee,nproc-1,13, &
                MPI_COMM_WORLD,status,ierr)

            call MPI_SENDRECV(w(1,1,1),1, &
                plantypee,nproc-1,16, &
                w(1,1,0),1, &
                plantypee,nproc-1,15, &
                MPI_COMM_WORLD,status,ierr)
    ...
    if(leftpem /= MPI_PROC_NULL) then
        call MPI_SSEND(u(0,0,kparasta),(jx+2)*(jy+2), &
            MPI_REAL_SD,leftpem,tagls, &
            MPI_COMM_WORLD,ierr)
        call MPI_SSEND(v(0,0,kparasta),(jx+2)*(jy+2), &
            MPI_REAL_SD,leftpem,tagls, &
            MPI_COMM_WORLD,ierr)
        call MPI_SSEND(w(0,0,kparasta),(jx+2)*(jy+2), &
            MPI_REAL_SD,leftpem,tagls, &
            MPI_COMM_WORLD,ierr)
    endif
    if(rightpem /= MPI_PROC_NULL) then
        call MPI_RECV(u(0,0,kparaend+1),(jx+2)*(jy+2), &
            MPI_REAL_SD,rightpem,tagrr, &
            MPI_COMM_WORLD,status,ierr)
        call MPI_RECV(v(0,0,kparaend+1),(jx+2)*(jy+2), &
            MPI_REAL_SD,rightpem,tagrr, &
            MPI_COMM_WORLD,status,ierr)
        call MPI_RECV(w(0,0,kparaend+1),(jx+2)*(jy+2), &
            MPI_REAL_SD,rightpem,tagrr, &
            MPI_COMM_WORLD,status,ierr)
    endif
