module buffer_bodyforce_module
    ...
    real(kind=c_double),public,bind(C) :: corr_factor
    integer(kind=c_int),public,bind(C) :: ispon,kspon

    real,public :: ti_pom_old,ti_pom_new,ti_pom_fin
...
contains
    ...
    subroutine buffer_bodyforce(ti,ktime,bcsi,beta,bzet)
        ...
        if (myid==0) then
            write(*,*)'BUFFER TURBULENCE on'
            if (ibodybuffer1) write(*,*)'side 1 on'
            if (ibodybuffer2) write(*,*)'side 2 on'
            if (ibodybuffer5) write(*,*)'side 5 on'
            if (ibodybuffer6) write(*,*)'side 6 on'
        end if



        if (ti>ti_pom_new)ntpom=ntpom + 1
        !-----------------------------------------------------------------------
        ...
        if (ibodybuffer1) then
            call disturbance_face1(ktime,cx,cy,cz,nti)
        end if

        ! on face 2
        if (ibodybuffer2) then
            call disturbance_face2(ktime,cx,cy,cz,nti)
        end if

        ! on face 5
        if (ibodybuffer5) then
            if (myid==0) then
                call disturbance_face5(ktime,cx,cy,cz,nti)
            end if
        end if
        ...
        if (ibodybuffer6) then
            if (myid==nproc-1) then
                call disturbance_face6(ktime,cx,cy,cz,nti)
            end if
        end if
        ...
        call sponge_dumping(ispon,n2,n3/nproc,local_dump,bcsi_f1,beta_f1,bzet_f1)
        ...
        ! quando faccia la f2 dovrebbe bastare invertire la local_dump?
        call sponge_dumping(ispon,n2,n3/nproc,local_dump,bcsi_f2,beta_f2,bzet_f2)
        deallocate(local_dump)
        ...
        if (myid==0) then
            ...
            call sponge_dumping(n1,n2,kspon,local_dump,bcsi_f5,beta_f5,bzet_f5)
            deallocate(local_dump)
        end if
        ...
        if (myid==nproc-1) then
            ...
            call sponge_dumping(n1,n2,kspon,local_dump,bcsi_f6,beta_f6,bzet_f6)
            deallocate(local_dump)
        end if
    ...
    subroutine sponge_dumping(nx,ny,nz,local_dump,var_csi,var_eta,var_zet)
    ...
    subroutine sponge_dumping_visco(nx,ny,nz,local_dump,var_csi,var_eta,var_zet)
    ...
    subroutine disturbance_face1(ktime,cx,cy,cz,nti)
        ...
        !
        if (ktime >1 .and. ktime==nti*(ktime/nti)) then
            ntime=nti
        ...
        if (ktime==1 .or. ktime==nti*(ktime/nti)) then
        ...
        call creo_xyz_f1(n2,kparasta,kparaend,coef_x,coef_y,coef_z,ntime)
            ...
            do j=1,jy
                do i=1,ispon
                    ...
                    if (index_out1(j,k)>0.01) then
                        ...
                        !         if (coef_x(i,j,k)<0.)write(420+myid,*)coef_x(i,j,k)
                        !         if (coef_x(i,j,k)>0.)write(430+myid,*)coef_x(i,j,k)

                        if (abs(uprime)<0.1*velrms ) then
                            uprime=0.1*segno_u*velrms
                        end if
                        if (abs(vprime)<0.1*velrms ) then
                            vprime=0.1*segno_v*velrms
                        end if
                        if (abs(wprime)<0.1*velrms ) then
    ...
    subroutine creo_xyz_f1(n2,kparasta,kparaend,coef_x,coef_y,coef_z,ntime)

        implicit none
        !-----------------------------------------------------------------------
        integer i,j,k,cont,ntime
        integer n2,kparasta,kparaend
    ...
    subroutine disturbance_face2(ktime,cx,cy,cz,nti)
        ...
        ntime=ktime -(ktime/nti)*nti
        call creo_xyz_f2(n1,n2,kparasta,kparaend,coef_x,coef_y,coef_z,ntime)
    ...
    subroutine creo_xyz_f2(n1,n2,kparasta,kparaend,coef_x,coef_y,coef_z,ntime)

        implicit none
        !-----------------------------------------------------------------------
        integer i,j,k,cont,icont,ntime
        integer n1,n2,kparasta,kparaend
    ...
    subroutine disturbance_face5(ktime,cx,cy,cz,nti)
        ...
        if (ktime==1) then
            ...
            noise_f5_zspace=0.
            call genero_random(corr_factor_sp,nreal,nstep,noise_f5_zspace,myid)
        end if
        !-----------------------------------------------------------------------
        !     to have a colored noise connected to the previous number generation
        !
        ...
        ntime=ktime -(ktime/nti)*nti
        call creo_xyz_f5(n1,n2,coef_x,coef_y,coef_z,ntime)
    ...
    subroutine creo_xyz_f5(n1,n2,coef_x,coef_y,coef_z,ntime)
    ...
    subroutine disturbance_face6(ktime,cx,cy,cz,nti)
        ...
        if (ktime==1) then
            ...
            noise_f6_zspace=0.
            call genero_random(corr_factor_sp,nreal,nstep,noise_f6_zspace,myid)
        end if
        !-----------------------------------------------------------------------
        !     to have a colored noise connected to the previous number generation
        !
        ...
        ntime=ktime -(ktime/nti)*nti
        call creo_xyz_f6(n1,n2,n3,coef_x,coef_y,coef_z,ntime)
    ...
    subroutine creo_xyz_f6(n1,n2,n3,coef_x,coef_y,coef_z,ntime)
    ...
    subroutine genero_random(corr_factor,nreal,nstep,disturbance,myid)
        !***********************************************************************
        !
        implicit none
